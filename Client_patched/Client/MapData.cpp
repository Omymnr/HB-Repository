...
int CMapData::iObjectFrameCounter(char * cPlayerName, short sViewPointX, short sViewPointY)
{
 ...
 static DWORD S_dwUpdateTime = 0;
 int   sWeaponType, sCenterX, sCenterY, sDist;//, sAbsX, sAbsY;
 BOOL  bAutoUpdate = FALSE, dynObjsNeedUpdate = FALSE;
 short dx, dy;
 long  lPan;
 
 // Variables para optimizar el rango de procesamiento
 int minX, maxX, minY, maxY;

	iRet = 0;
	dwTime = dwRealTime = m_pGame->m_dwCurTime;  // Usar tiempo cacheado
	...
	// Optimización: Solo procesar tiles visibles + margen
	minX = (sCenterX > 15) ? (sCenterX - 15) : 0;
	maxX = (sCenterX + 15 < MAPDATASIZEX) ? (sCenterX + 15) : MAPDATASIZEX;
	minY = (sCenterY > 12) ? (sCenterY - 12) : 0;
	maxY = (sCenterY + 12 < MAPDATASIZEY) ? (sCenterY + 12) : MAPDATASIZEY;
			...
			if ((dwTime - m_pData[dX][dY].m_dwOwnerTime) > dwFrameTime)
			{
				...
				{	iRet = 1;
					...
					if ((dwRealTime - m_dwFrameCheckTime) > dwFrameTime)
					{
						...
						// Limitar para evitar acumulación excesiva
						if (m_dwFrameAdjustTime > 100) m_dwFrameAdjustTime = 100;
					}
