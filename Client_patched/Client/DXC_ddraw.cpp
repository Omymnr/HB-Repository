...
BOOL DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	{
		// Windowed Borderless Mode (Fullscreen Windowed)
		...
		SetWindowPos(hWnd, HWND_TOP, 0, 0, iScreenWidth, iScreenHeight, SWP_SHOWWINDOW);
		...
		SetRect(&m_rcFlipping, 0, 0, iScreenWidth, iScreenHeight); // Stretch to fill screen
	}
	...
	if (hResult == DD_OK)
	{
	   if (ddSurfaceDesc2.ddpfPixelFormat.dwRBitMask == 0x0000F800) {
		  ...
		  // BGR 5:6:5 
		   ...
		   // Default to RGB 5:6:5 for modern systems (32-bit desktop)
		   m_cPixelFormat = 1;
	   }
	}
	else {
		// Fallback to RGB 5:6:5 if detection fails
		m_cPixelFormat = 1;
	}
}
...
{
	// Usar ColorFill de DirectDraw - más eficiente que Lock/memset/Unlock
	DDBLTFX ddbltfx;
	ZeroMemory(&ddbltfx, sizeof(ddbltfx));
	ddbltfx.dwSize = sizeof(ddbltfx);
	ddbltfx.dwFillColor = 0;  // Negro
	
	if (m_lpBackB4->Blt(NULL, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT, &ddbltfx) != DD_OK)
	{
		// Fallback al método antiguo si ColorFill falla
		DDSURFACEDESC2 ddsd2;
		ddsd2.dwSize = sizeof(ddsd2);
...
void DXC_ddraw::SetFullMode(BOOL bFullMode)
{
