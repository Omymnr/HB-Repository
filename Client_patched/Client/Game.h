...
{
public:
	void DrawCursor(int x, int y, int frame, DWORD time);
	// CLEROTH - AURAS
	...
	void NotifyMsg_SettingSuccess(char * pData); // OHMYWEED - LU
...
//Snoopy end<<<<<<<<<<<<<<<<<<
	...
	BOOL m_bEndlessRiftBlocked;  // TRUE = no puede entrar al Endless Rift (evento activo por otra party)
