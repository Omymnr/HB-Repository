...
void CSprite::PutSpriteScaled(int sX, int sY, int sFrame, int sWidth, int sHeight, DWORD dwTime)
{
	short dX, dY, sx, sy, szx, szy, pvx, pvy;
	RECT rcSrc, rcDest;
	...
	// Scaling factors
	float fScaleX = (float)sWidth / (float)origWidth;
	float fScaleY = (float)sHeight / (float)origHeight;

	dX = sX + (int)(pvx * fScaleX);
	dY = sY + (int)(pvy * fScaleY);

	// Clipping
	int destL = dX;
	int destT = dY;
	int destR = dX + sWidth;
	int destB = dY + sHeight;
	...
	// Clip Left
	if (destL < m_pDDraw->m_rcClipArea.left) {
		int diff = m_pDDraw->m_rcClipArea.left - destL;
		destL = m_pDDraw->m_rcClipArea.left;
		srcL += (int)(diff / fScaleX);
	}
	// Clip Top
	if (destT < m_pDDraw->m_rcClipArea.top) {
		int diff = m_pDDraw->m_rcClipArea.top - destT;
		destT = m_pDDraw->m_rcClipArea.top;
		srcT += (int)(diff / fScaleY);
	}
	// Clip Right
	if (destR > m_pDDraw->m_rcClipArea.right) {
		int diff = destR - m_pDDraw->m_rcClipArea.right;
		destR = m_pDDraw->m_rcClipArea.right;
		srcR -= (int)(diff / fScaleX);
	}
	// Clip Bottom
	if (destB > m_pDDraw->m_rcClipArea.bottom) {
		int diff = destB - m_pDDraw->m_rcClipArea.bottom;
		destB = m_pDDraw->m_rcClipArea.bottom;
		srcB -= (int)(diff / fScaleY);
	}

	if (destL >= destR || destT >= destB) {
		m_bOnCriticalSection = FALSE;
		return;
	}
	...
	m_pDDraw->m_lpBackB4->Blt(&rcDest, m_lpSurface, &rcSrc, DDBLT_KEYSRC | DDBLT_WAIT, NULL);
