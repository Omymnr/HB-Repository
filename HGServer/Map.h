...
// Elite Mob System
#define DEF_ELITEMOB_KILL_THRESHOLD		100  // Kills needed to spawn elite mob
#define DEF_ELITEMOB_MAXNPCTYPES		500  // Max NPC types to track

// Barracks Elite Event System
#define DEF_BARRACKS_DUMMY_KILL_THRESHOLD	150  // Kills needed to spawn elite mobs
#define DEF_BARRACKS_ELITE_LIFETIME_MS		120000  // 60 seconds lifetime for elite mobs
#define DEF_BARRACKS_ELITE_COUNT			2      // Number of elite mobs to spawn
...
public:
	...
	BOOL  m_bNamingValueUsingStatus[50000]; // 0~49999 - Massive limit for unlimited NPC spawning per map
	...
	// Elite Mob System - Kill counter per NPC type in this map
	int m_iEliteMobKillCount[DEF_ELITEMOB_MAXNPCTYPES];

	// Barracks Elite Event System
	BOOL  m_bIsBarracksMap;           // TRUE if this is a barracks map (arebrk/elvbrk)
	int   m_iBarracksDummyKillCount;  // Counter for Dummy/Attack-Dummy kills
	BOOL  m_bBarracksEliteActive;     // TRUE if elite mobs are currently spawned
	DWORD m_dwBarracksEliteSpawnTime; // Time when elite mobs were spawned
	int   m_iBarracksEliteNpcH[DEF_BARRACKS_ELITE_COUNT]; // NPC handles of spawned elites
