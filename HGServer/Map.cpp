...
: m_bIsSnowEnabled(FALSE)
{
 int i, ix, iy;
	...
	// Elite Mob System - Initialize kill counters
	for (i = 0; i < DEF_ELITEMOB_MAXNPCTYPES; i++)
		m_iEliteMobKillCount[i] = 0;
	...
	// Barracks Elite Mob System - Initialize
	m_iBarracksDummyKillCount = 0;
	m_bBarracksEliteActive = FALSE;
	m_dwBarracksEliteSpawnTime = 0;
	for (i = 0; i < DEF_BARRACKS_ELITE_COUNT; i++)
		m_iBarracksEliteNpcH[i] = 0;
	m_bIsBarracksMap = FALSE;
}
...
BOOL CMap::bInit(char * pName)
{
 int i;
	...
	// Barracks Elite Mob System - Detect Barracks maps
	if ((memcmp(pName, "arebrk", 6) == 0) || (memcmp(pName, "elvbrk", 6) == 0)) {
		m_bIsBarracksMap = TRUE;
	}
...
BOOL CMap::_bDecodeMapDataFileContents()
{
 ...
 short * sp;
	...
	hFile = CreateFileA(cMapFileName, GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
